...
Note! It is convention to store a single class, or struct in a file with the same as the class or struct defined in that file. 

Note! It is convention to name classes and structs beginning with an uppercase letter. 
...
> [solution]
> Replace `name` with `habits`. Notice that the title displayed in the cell gets it value from `habit.title` this is the "key" where the `Habit` struct stores a string. 
...
> Find the `pressAddHabit(sender:)` method. This still refers to the array `names` you need to update this to `habits`. It also adds a string but `habits` array needs a `Habit`. Add a new `Habit` to the `Habits` array:
...
To make the app useful it would good store more data with each habit. Since you used a struct, which is a collection type, you can easily expand the number of properties. 
...
Update the `Habit` struct with the new properties. Notice that these are variables that are stored in the struct. Each time you make a new instance of this struct that instance stores all of these variables with their own values. 
...
`dateCreated` can't be changed. Makes sense becuase if you create a habit on Monday, you can't say you created it on Thursday at some point in the future!

`numberOfCompletions` on the other hand will be updated each time you complete a habit. 

Note! `lastCompletionDate` is type `Date?` this is an optional. Which means it might be a `Date` or it might be `nil`. Think about it like this. When you decide to brush your teeth as a habit, you haven't brushed them yet, so the `lastCompletionDate` doesn't exist. Later that day when you brush your teeth, you update your habit tracker and the `lastCompletionDate` is created. 

What if you decided that climbing mount everest was a good habit! It might go uncompleted for a long time, you might never completed it! 

We use optionals in Swift for situations where a variable might have a value but might also not have a value. 

## Dates

The `Date` class is built in and has a lot of methods and features. A date keeps track of a moment in time, that includes the year, the month, the date, the day of the week, the time of day including the hour, minutes, seconds, and more! 
...
- `stringvalue` - This returns a human readable date string from the a `Date` instance
...
The icons come in several resolutions, x1, x2, and x3, for different screen sizes. You'll to tell Xcode that which icon is which resolution. 

> [action]
> Download [these icons](./assets/Icons.zip) and unzip them if they are not unzipped. 

> Add the x1 icons to **Assets.xcassets**. Then add the x2 to version of the icon and the x3 version. Like this: 
...
An enum is a data type that represents a fixed number of items that are always the same items. These are things like days of the week, which are always 7 and always have the same names.
...
> [solution]
> Edit `Habit.swift`. Look at the code below find the similar code and make these changes. If code is not shown leave it as is. 
