...
You are now going to delve into the world of saving data in your application! **Persistence** is a key component that allows us to manage state for an application.

With persistence we are trusting the application to store data even after the user exits out of our application. 
...
> [action] Lets add this function to our persistence layer. Add the method below to the `PersistenceLayer` class.
...
Much of the code may apear to be very opaque at the moment. I'll summarize. PersistanceLayer is a struct. It has a couple variables and methods. 

The habits veriable contains the list of habits. 

PersistenceLayer loads and saves the habits array to a "place" called NSUserDefaults. This a feature built into the phone that allows developers to store data outside of their apps. 
...
> [action] 
> Lets add this function to our persistence layer. Add the code below to the `PersistenceLayer` struct.
